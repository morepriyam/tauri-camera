...
const MAX_TOTAL_DURATION = 60 * 1000; // 60 seconds in milliseconds
...
export default function Camera({ onError }: CameraProps) {
  ...
  // Consolidated state logging effect
  useEffect(() => {
    console.log("📊 [State] App state changed:", {
      // Recording state
      isRecording,
    ...
    try {
      ...
      if (stream) {
        ...
        stream.getTracks().forEach((track, index) => {
          console.log(
            `🎥 [Camera] Stopping track ${index}: ${track.kind} (${track.label})`
          );
          track.stop();
        });
        ...
        if (videoRef.current) {
          console.log(
            "🎥 [Camera] Clearing video element source before restarting"
          );
      ...
      console.log(
        `🎥 [Camera] Stream tracks: ${mediaStream
          .getTracks()
      ...
      if (videoRef.current) {
        console.log(
          "🎥 [Camera] Setting video element source and starting playback"
        );
      ...
      console.log(
        `🎥 [Camera] Camera successfully initialized with facing mode: ${facing}`
      );
    } catch (err) {
      ...
      if (err instanceof Error) {
        ...
        if (err.name === "NotAllowedError") {
          ...
          try {
            ...
            console.log(
              "🎥 [Camera] Attempting fallback with basic constraints"
            );
            ...
            setFacingMode(facing);
            console.log(
              "🎥 [Camera] Fallback camera initialization successful"
            );
  ...
  const flipCamera = async () => {
    ...
    console.log(
      `🔄 [Camera] Flipping camera from ${facingMode} to ${newFacing}`
    );
    ...
    try {
      ...
      console.log(
        `🔄 [Camera] Attempting to restore ${facingMode} camera after flip failure`
      );
      try {
        await startCamera(facingMode);
      } catch (restoreError) {
        console.error(
          "🔄 [Camera] Failed to restore original camera:",
          restoreError
        );
  ...
  const startRecording = useCallback(() => {
    ...
    console.log(
      `🔴 [Recording] Current state - isRecording: ${isRecording}, totalDuration: ${totalDuration}ms, maxDuration: ${MAX_TOTAL_DURATION}ms`
    );

    if (!stream) {
      console.error(
        "🔴 [Recording] Cannot start recording - no stream available"
      );
      return;
    }
    ...
    if (totalDuration >= MAX_TOTAL_DURATION) {
      console.warn(
        "🔴 [Recording] Cannot start recording - max duration reached"
      );
      return;
    }
    ...
    try {
      ...
      mediaRecorder.ondataavailable = (event) => {
        if (event.data.size > 0) {
          ...
          console.log(
            `🔴 [Recording] Added chunk - total: ${chunks.length}, size: ${event.data.size} bytes`
          );
        }
      };
      ...
      mediaRecorder.onstop = () => {
        ...
        console.log(
          `🔴 [Recording] Created blob - size: ${blob.size} bytes, duration: ${duration}ms`
        );
        ...
        setVideoSegments((prev) => {
          ...
          console.log(
            `🔴 [Recording] Updated segments array - total segments: ${newSegments.length}`
          );
          return newSegments;
        });
        ...
        setTotalDuration((prev) => {
          ...
          console.log(
            `🔴 [Recording] Updated total duration: ${prev}ms -> ${newTotal}ms`
          );
          return newTotal;
        });
      ...
      setIsRecording(true);
      console.log(
        `🔴 [Recording] Recording started at timestamp: ${recordingStartTimeRef.current}`
      );
      ...
      recordingIntervalRef.current = setInterval(() => {
        ...
        if (totalDuration + elapsed >= MAX_TOTAL_DURATION) {
          console.log(
            `🔴 [Recording] Auto-stopping - reached max duration (${
              totalDuration + elapsed
            }ms >= ${MAX_TOTAL_DURATION}ms)`
          );
          stopRecording();
        }
      }, 100);
  ...
  const stopRecording = useCallback(() => {
    ...
    console.log(
      `⏹ [Recording] Current state - isRecording: ${isRecording}, mediaRecorder exists: ${!!mediaRecorderRef.current}`
    );
    ...
    if (mediaRecorderRef.current && isRecording) {
      ...
      console.log(
        "⏹ [Recording] MediaRecorder stopped and cleared, isRecording set to false"
      );
    } else {
      console.warn(
        "⏹ [Recording] Stop recording called but not currently recording"
      );
    }
    ...
    setVideoSegments((prev) => {
      ...
      if (segmentToDelete) {
        ...
        setTotalDuration((total) => {
          ...
          console.log(
            `🗑 [Segments] Updated total duration: ${total}ms -> ${newTotal}ms`
          );
          return newTotal;
        });
      } else {
      ...
      console.log(
        `🗑 [Segments] Segments after deletion: ${filteredSegments.length} remaining`
      );
  ...
  const enterPreviewMode = () => {
    ...
    if (videoSegments.length === 0) {
      console.warn(
        "▶️ [Preview] Cannot enter preview mode - no segments available"
      );
      return;
    }
    ...
    if (stream) {
      ...
      stream.getTracks().forEach((track) => {
        console.log(
          `▶️ [Preview] Stopping track: ${track.kind} (${track.label})`
        );
        track.stop();
      });
    ...
    console.log(
      "▶️ [Preview] Entering preview mode, setting currentPreviewIndex to 0"
    );
  ...
  const exitPreviewMode = async () => {
    ...
    setCurrentPreviewIndex(0);
    console.log(
      "⬅️ [Preview] Exited preview mode, reset currentPreviewIndex to 0"
    );
  ...
  const playNextSegment = () => {
    console.log(
      `⏭ [Preview] Play next segment - current index: ${currentPreviewIndex}, total segments: ${videoSegments.length}`
    );
  ...
  useEffect(() => {
    if (
      ...
      videoSegments[currentPreviewIndex]
    ) {
      ...
      playPromise
        .then(() => {
          console.log(
            `📹 [Preview] Successfully started playing segment ${
      ...
      const handleEnded = () => {
        console.log(
          `📹 [Preview] Segment ${currentPreviewIndex + 1} finished playing`
        );
        playNextSegment();
      };
      ...
      const handleLoadStart = () => {
        console.log(
          `📹 [Preview] Started loading segment ${currentPreviewIndex + 1}`
        );
      };

      const handleCanPlay = () => {
        console.log(
          `📹 [Preview] Segment ${currentPreviewIndex + 1} ready to play`
        );
      };
      ...
      return () => {
        console.log(
          `📹 [Preview] Cleaning up event listeners for segment ${
            currentPreviewIndex + 1
          }`
        );
  ...
  useEffect(() => {
    if (isPreviewMode && isRecording) {
      console.log(
        "▶️ [Preview] Stopping recording because preview mode was entered"
      );
      stopRecording();
    }
  ...
  useEffect(() => {
    console.log(
      "🚀 [Lifecycle] Camera component mounting, initializing camera"
    );
    startCamera();

    return () => {
      console.log(
        "🧹 [Lifecycle] Camera component unmounting, cleaning up resources"
      );
      ...
      if (stream) {
        ...
        stream.getTracks().forEach((track, index) => {
          console.log(
            `🧹 [Lifecycle] Stopping track ${index}: ${track.kind} (${track.label})`
          );
          track.stop();
        });
      }
      ...
      // Cleanup video URLs
      console.log(
        `🧹 [Lifecycle] Cleaning up ${videoSegments.length} video segment URLs`
      );
      videoSegments.forEach((segment, index) => {
        console.log(
          `🧹 [Lifecycle] Revoking URL for segment ${index + 1}: ${segment.url}`
        );
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      console.log(
        `👁 [Visibility] Page visibility changed - hidden: ${document.hidden}`
      );

      if (document.hidden) {
        console.log(
          "👁 [Visibility] Page became hidden, pausing video and stopping recording if active"
        );
        ...
        if (isRecording) {
          console.log(
            "👁 [Visibility] Stopping active recording due to visibility change"
          );
          stopRecording();
        }
      } else {
        console.log(
          "👁 [Visibility] Page became visible, resuming video playback"
        );
        ...
        if (videoRef.current && stream) {
          ...
          playPromise.catch((err) => {
            console.error(
              "👁 [Visibility] Failed to resume video playback:",
              err
            );
          });
        }
  ...
  const getRemainingTime = useCallback(() => {
    const remaining =
      MAX_TOTAL_DURATION - totalDuration - currentRecordingDuration;
    const remainingTime = Math.max(0, remaining);
    console.log(
      `⏱ [Duration] Remaining time calculated: ${remainingTime}ms (total: ${totalDuration}ms, current: ${currentRecordingDuration}ms, max: ${MAX_TOTAL_DURATION}ms)`
    );
    return remainingTime;
  }, [totalDuration, currentRecordingDuration]);
  ...
  if (isPreviewMode) {
    console.log(
      `▶️ [UI] Rendering preview mode - segment ${currentPreviewIndex + 1}/${
        videoSegments.length
      }`
    );
    return (
    ...
    <div className="camera-container">
      ...
      <div className="camera-overlay">
        ...
        <div className="top-controls">
          ...
          <div className="recording-info">
            <div className="total-duration">
              ...
              {formatDuration(MAX_TOTAL_DURATION)}
            </div>
            {isRecording && (
            ...
            <div className="segments-progress-bar">
              {videoSegments.map((segment) => (
                <div
                  ...
                  style={{
                    width: `${(segment.duration / MAX_TOTAL_DURATION) * 100}%`,
                  }}
                />
              ))}
              {isRecording && (
                <div
                  ...
                  style={{
                    width: `${
                      (currentRecordingDuration / MAX_TOTAL_DURATION) * 100
                    }%`,
                  }}
                />
              )}
        ...
        <div className="bottom-controls">
          ...
          >
            <div className="record-button-inner">{isRecording ? "⏹" : "⏺"}</div>
